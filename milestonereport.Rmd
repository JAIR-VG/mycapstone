...
The three datasets are partitioned into a training set and a test set. This approach is called holdout, where the training set is used for building machine learning models and the test set is to evaluate using a different data. Here, we will use 30% for training and the remaining for testing.

```{r holdout, echo=TRUE}
linblogs <-sample(linblogs, length(linblogs)*0.30)
linews <-sample(linnews, length(linnews)*0.30)
...
## Basic Processing
We perform some preprocessing on each dataset. Here, we use two kinds of processing instructions.
```{r corpus, echo=TRUE}
